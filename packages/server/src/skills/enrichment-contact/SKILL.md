---
id: crm.enrichment-contact
priority: 50
roles: [enrichment-contact]
requires:
  - crm.contacts.get
  - crm.contacts.update
  - crm.companies.get
...
Response: `{"ok": true, "result": { ...contact... }}`.

If the contact has a `companyId`, fetch the company too:
...
Write the structured JSON dossier to `customFields.dossier` via the update tool:

```
curl -X POST "$BORINGOS_CALLBACK_URL/api/tools/crm.contacts.update" \
  -H "Authorization: Bearer $BORINGOS_CALLBACK_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
    "id": "CONTACT_ID",
...
Response: `{"ok": true, "result": { ...updated contact... }}`.
...
- Mark each task as done after writing the dossier (use `framework.tasks.patch` with `{"status": "done"}`).
...
curl -X POST "$BORINGOS_CALLBACK_URL/api/tools/framework.tasks.create" \
  ...
  -d '{
    ...
    "assigneeUserId": "<contact owner user id>",
...
1. **Read the comment** — fetch the task via `framework.tasks.read` (`{"taskId": "..."}`); the response includes its comments. The user's reply is the answer to the question in the task title.
2. **Find the related contact** — use the task's `parentId` (chains back to the enrichment task, which references the contact) or the contact referenced in the task title/body.
3. **Fetch the current dossier** via `crm.contacts.get` and update it:
...
4. **Write the updated dossier back** via `crm.contacts.update` (set `customFields.dossier` to the merged dossier).
5. **If the reply changes the picture materially** — e.g., switches intent from investor to prospect — propose 1–2 new `human_todo` or `agent_action` tasks (via `framework.tasks.create`) that make sense under the new framing. Skip if the reply is just confirmation.
6. **Post a short comment on the task** via `framework.comments.post` summarizing what you updated (1–2 sentences), then **mark the task done** via `framework.tasks.patch` with `{"taskId": "...", "status": "done"}`.
